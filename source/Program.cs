...
namespace SmbManager
{
    ...
    {
        private const string AppTitle = "SMB Manager V1.6.8";
        private const string DefaultServerHost = "192.168.1.65";
        private const string CurrentVersionText = "V1.6.8";
        private const int AdminPasswordIterations = 120000;
        private const string UpdateShareName = "지점공용";
        private const string UpdateFolderName = "App Update";
        private const string UpdateManifestName = "latest.ini";
        ...
        {
            var publicFolder = _folders.FirstOrDefault(folder => folder.Key == UpdateShareName);
            var share = publicFolder == null ? UpdateShareName : publicFolder.Share;
            return "\\\\" + _serverHost + "\\" + share + "\\" + UpdateFolderName;
        }

        private string ResolveUpdateFolderPath()
        {
            var unc = GetUpdateFolderPath();
            ...
            var publicFolder = _folders.FirstOrDefault(folder => folder.Key == UpdateShareName);
            if (publicFolder != null && !string.IsNullOrWhiteSpace(publicFolder.Drive))
            {
                var drivePath = Path.Combine(publicFolder.Drive + "\\", UpdateFolderName);
        ...
        private static string GetIniValue(Dictionary<string, string> values, string key)
        {
            string value;
            return values.TryGetValue(key, out value) ? value : string.Empty;
        }

        private static void CreateUpdateManifestTemplate(string path)
        {
            var text =
                "Version=V1.6.8\r\n" +
                "FileName=SMB Manager V1.6.8.exe\r\n" +
                "Message=최신 버전 안내 문구를 입력하세요.\r\n";
        ...
        private void CheckForUpdates(bool automatic)
        {
            try
            {
                var updateFolder = ResolveUpdateFolderPath();
                ...
                var manifestPath = Path.Combine(updateFolder, UpdateManifestName);
                if (!File.Exists(manifestPath))
                {
                    ...
                    var createTemplate = MessageBox.Show(
                        "업데이트 정보 파일이 없습니다.\r\n\r\n" + manifestPath + "\r\n\r\n기본 양식 파일을 생성할까요?",
                        AppTitle,
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question);

                    if (createTemplate == DialogResult.Yes)
                    {
                        CreateUpdateManifestTemplate(manifestPath);
                        Process.Start("notepad.exe", manifestPath);
                    }
                    return;
                }

                var manifest = ReadSimpleIni(manifestPath);
                var latestVersion = GetIniValue(manifest, "Version");
                if (string.IsNullOrWhiteSpace(latestVersion))
                {
                    ...
                    MessageBox.Show("업데이트 정보 파일에 Version 값이 없습니다.", AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                var comparison = CompareVersionText(latestVersion, CurrentVersionText);
                if (comparison <= 0)
                {
                    if (automatic)
                    {
                ...
                var fileName = GetIniValue(manifest, "FileName");
                if (string.IsNullOrWhiteSpace(fileName))
                {
                    if (!automatic)
                    {
                        MessageBox.Show("업데이트 정보 파일에 FileName 값이 없습니다.", AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    return;
                }

                var updateFilePath = Path.Combine(updateFolder, fileName);
                if (!File.Exists(updateFilePath))
                {
                    if (!automatic)
                    {
                        MessageBox.Show("업데이트 파일을 찾을 수 없습니다.\r\n\r\n" + updateFilePath, AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    return;
                }

                var message = GetIniValue(manifest, "Message");
                ...
                prompt += "\r\n\r\n지금 업데이트하고 새 버전으로 다시 실행할까요?";

                AppendResultLog("성공: 업데이트 확인 / 새 버전 " + latestVersion);
                if (automatic)
                {
                    if (_automaticUpdate)
                    {
                        Log("새 버전을 자동 설치합니다: " + latestVersion);
                        InstallUpdateAndRestart(updateFilePath, latestVersion, fileName, true);
                    }
                    else
                    {
                        Log("새 버전이 있습니다: " + latestVersion);
                    }
                ...
                {
                    InstallUpdateAndRestart(updateFilePath, latestVersion, fileName, false);
                }
            }
            catch (Exception error)
            {
        ...
        private void InstallUpdateAndRestart(string updateFilePath, string latestVersion, string fileName, bool silent)
        {
            var installRoot = GetLocalUpdateInstallRoot();
            var versionDir = Path.Combine(installRoot, MakeSafeFileName(latestVersion));
            Directory.CreateDirectory(versionDir);

            var destinationPath = Path.Combine(versionDir, fileName);
            File.Copy(updateFilePath, destinationPath, true);
            CopyCurrentCompanionFiles(versionDir);
            File.WriteAllText(Path.Combine(installRoot, "current.txt"), destinationPath, Encoding.UTF8);
            CleanupPreviousInstalledVersions(versionDir);
            CreateDesktopShortcut(destinationPath, versionDir);

            AppendResultLog("성공: 업데이트 설치 / " + latestVersion);
            if (!silent)
            {
                MessageBox.Show("업데이트 파일을 설치했습니다.\r\n바탕화면에 실행 바로가기를 만들었습니다.\r\n\r\n앞으로는 기존 공유폴더의 예전 파일이 아니라 바탕화면 바로가기로 실행해 주세요.\r\n새 버전으로 다시 실행합니다.", AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        ...
        private static void CopyCurrentCompanionFiles(string targetDir)
        {
            ...
            CopyCurrentFileIfExists("version.ini", targetDir);
    ...
    internal sealed class AdminSettingsForm : Form
    {
        ...
        private TabPage BuildGeneralTab(string serverHost, bool automaticUpdate, bool monitorEnabled, int monitorIntervalSeconds)
        {
            ...
            _automaticUpdateBox = new CheckBox { Text = "실행 시 자동 업데이트", AutoSize = true, Checked = automaticUpdate };
